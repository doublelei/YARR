...
class UniformReplayBuffer(ReplayBuffer):
    ...
    def __init__(self,
        ...
        self._task_index = {}
        self._num_tasks = 0
        self._task_start_index = {}
        self._task_end_index = {}

        self._replay_index_by_task = []
        self._replay_weight_by_task = []
    ...
    def recover_from_disk(self, task, task_replay_storage_folder):
        ...
        self.end_indices = np.where(self._store[TERMINAL] == 1)[0]
        self.start_indices = np.concatenate(([0], self.end_indices[:-1] + 1))

    def recover_from_disk_wo_repeat(self, task, task_replay_storage_folder):
        if os.path.exists(os.path.join(task_replay_storage_folder, 'replay_info.npy')):
            with open(os.path.join(task_replay_storage_folder, 'replay_info.npy'), 'rb') as fp:
                replay_info = np.load(fp, allow_pickle = True).item()
        else:
            raise ValueError('replay_info.npy does not exist')
        ...
        for i in range(len(replay_info['terminal'])):
            cursor = self.cursor()
            self._store[TERMINAL][cursor] = replay_info['terminal'][i]
        ...
        self.end_indices = np.where(self._store[TERMINAL] == 1)[0]
        self.start_indices = np.concatenate(([0], self.end_indices[:-1] + 1))
        _replay_index_by_task = []
        for i in replay_info['sample_points_replay_index']:
            _replay_index_by_task.append(i + self._task_replay_start_index[-1])
        
        for i in replay_info['key_points_replay_index']:
            _replay_index_by_task.append(i + self._task_replay_start_index[-1])
        self._replay_index_by_task.append(_replay_index_by_task)
        _replay_weight_by_task = np.array(replay_info['sample_points_replay_weights'] + replay_info['key_points_replay_weights'])
        self._replay_weight_by_task.append(_replay_weight_by_task / sum(_replay_weight_by_task))
    ...
    def is_valid_transition(self, index, distribution_mode):
        ...
        if distribution_mode == 'vip' or distribution_mode == 'vip_vis':
        ...
        # if distribution_mode == 'task_uniform_2':
        #     return True # dont check terminal
    ...
    def sample_index_batch(self, batch_size, distribution_mode = 'transition_uniform'):
        ...
        if distribution_mode == 'transition_uniform':
            ...
            while (len(indices) < batch_size and
                ...
                if self.is_valid_transition(index, distribution_mode):
            ...
            for task_index in task_indices:
                ...
                while attempt_count < self._max_sample_attempts:
                    # print(self._task_replay_start_index[task_index] + self._task_add_count[task_index].value)
                    ...
                    if self.is_valid_transition(state_index, distribution_mode):
                        indices.append(state_index)
                        break
                    else:
                        attempt_count += 1
        elif distribution_mode == 'weighted_keypoints':
            attempt_count = 0
            while (len(indices) < batch_size and
                        attempt_count < self._max_sample_attempts):
                index = np.random.choice(self._all_index, p=self._all_weights) % self._replay_capacity
                # sample_from_keypoints = np.random.random()
                # if sample_from_keypoints < 0.95:
                #     index = np.random.choice(self._key_points_replay_index) % self._replay_capacity
                # else:
                #     index = np.random.choice(self._sample_points_replay_index) % self._replay_capacity
                if self.is_valid_transition(index, distribution_mode):
                    indices.append(index)
                else:
                    attempt_count += 1
        elif distribution_mode == 'weighted_keypoints_task':
            attempt_count = 0
            task_indices = np.random.randint(low = 0, high = self._num_tasks, size = batch_size)
            for task_index in task_indices:
                attempt_count = 0
                while attempt_count < self._max_sample_attempts:
                    index = np.random.choice(self._replay_index_by_task[task_index], p=self._replay_weight_by_task[task_index]) % self._replay_capacity
                    if self.is_valid_transition(index, distribution_mode):
                        indices.append(index)
                        break
                    else:
                        attempt_count += 1
        elif distribution_mode == 'task_uniform_2':
            task_indices = np.random.randint(low = 0, high = self._num_tasks, size = batch_size // 2)
            for task_index in task_indices:
                attempt_count = 0
                while attempt_count < self._max_sample_attempts:
                    # print(self._task_replay_start_index[task_index] + self._task_add_count[task_index].value)
                    state_index = np.random.randint(low = self._task_replay_start_index[task_index], \
                                                    high = self._task_replay_start_index[task_index] + self._task_add_count[task_index].value - 1)
                    if self.is_valid_transition(state_index, distribution_mode) and self.is_valid_transition(state_index+1, distribution_mode) :
                        indices.append(state_index)
                        indices.append(state_index+1)
                        break
                    else:
                        attempt_count += 1

          
        elif distribution_mode == 'vip':
            attempt_count = 0
            assert batch_size % 4 == 0
            for _ in range(batch_size // 4):
                # Choose a random episode
                episode_idx = np.random.randint(len(self.start_indices))

                # randomly choose a task
                task_idx = np.random.randint(self._num_tasks)
                #pick episode from that task
                random_state_idx = np.random.randint(self._task_replay_start_index[task_idx], \
                                                self._task_replay_start_index[task_idx] + self._task_add_count[task_idx].value)

                # find correpsdoning episode index
                # by looking at the start and end indices with binary search
                episode_idx = np.searchsorted(self.start_indices, random_state_idx, side='right') - 1

                # Indices for first frame, last frame
                first_frame_index = self.start_indices[episode_idx]
                last_frame_index = self.end_indices[episode_idx]
                
                # Indices for two random middle consecutive frames
                # We check if the length of the episode is greater than 2
                # If not, the first and last frames will also be the middle frames
                if last_frame_index - first_frame_index > 1:
                    random_middle_index = np.random.randint(first_frame_index + 1, last_frame_index)
                    middle_frame_indices = (random_middle_index, random_middle_index + 1)
                else:
                    middle_frame_indices = (first_frame_index, last_frame_index)
                    
                indices.append(first_frame_index); indices.append(middle_frame_indices[0]); indices.append(middle_frame_indices[1]); indices.append(last_frame_index)
        
        elif distribution_mode == 'vip_vis':
            attempt_count = 0
            # assert batch_size % 4 == 0
            for _ in range(batch_size // 4):
                # Choose a random episode
                ...
                # append all indices between first and last frame
                for i in range(first_frame_index, last_frame_index + 1):
                    indices.append(i)
        else:
            raise NotImplementedError

        if len(indices) != batch_size and distribution_mode != 'vip_vis':
            raise RuntimeError(
    ...
    def sample_transition_batch(self, batch_size=None, indices=None,
        ...
        with self._lock:
            ...
            for batch_element, state_index in enumerate(indices):
                ...
                if not self.is_valid_transition(state_index, distribution_mode):
    ...
    def prepare_enumeration(self, distribution_mode):
        '''
